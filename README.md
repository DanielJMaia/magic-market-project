...
- Django messages when logging out was problematic. Rendering home page after logging out is a bad idea because the URL is still the logout URL, so if you refresh it it's a form resubmission problem. Plus, it's just not as nice to have a weird URL but to be on the home page. Redirecting it was much better. Django messages also don't fade the way I want them to. This is because I wanted to use jquery fade but the minified version of jquery you must use for bootstrap doesn't contain the fade classes, and I didn't succeed in finding a version of jquery that had the fade classes and also worked fully with bootstrap. It was always one or the other.
- Secondary key referencing. It's double underscores, not single. i.e. card__user_username
- Greater than/less than filtering in the advanced filtering section. It really doesn't like null fields, and trying to replace null values with 0 or 999999 in the backend was very difficult. This is the only bug I gave up on fixing and just added default values. Same result, ever so slightly less clean. 
- Viewing previous orders. Very annoying. This caused hours and hours of issues. The eventual fix was to clear all my previous migrations because a lot of my older objects were missing certain fields that I later added when I was needing more than the very basic fields like name. Basically I was trying to view order history by accessing the user lined to an order, but 90% of the orders I'd previously done while testing didn't have a user column. I couldn't even get into the admin panel so I had to delete all previous migrations. Basically user column didn't exist so it was giving me errors saying it was running into a problem with the user column. The biggest pain was that it was a database error so even when I deleted all the problematic code, I was still getting the same error referencing things that didn't even exist anymore!
- Media image URLs change when you upload everything to an S3 bucket. I didn't realise this, and spent a long time wondering why my site was getting my static files from S3 and my media files from GitPod. I realised later that I had to change the way I was referencing images in the HTML
